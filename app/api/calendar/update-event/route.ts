...
export async function POST(req: NextRequest) {
  ...
  const { exerciseId, calendarEventId, type, date, time, durationMinutes, notes, timeZone } = await req.json()
  ...
  const emoji: Record<string, string> = { Run: '🏃', Gym: '🏋️', Yoga: '🧘', Cycling: '🚴', Swim: '🏊', Walk: '🚶', HIIT: '⚡', Pilates: '🤸', Other: '💪' }
  ...
  const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events/${calendarEventId}`, {
    ...
    body: JSON.stringify({
      ...
      description: [notes, `Duration: ${durationMinutes} min`, 'Added via Daybook'].filter(Boolean).join('\n'),
  ...
  if (!event.id) return NextResponse.json({ error: event.error?.message ?? 'Update failed' }, { status: 500 })

  await supabase.from('weekly_exercises').update({
    type, exercise_date: date, exercise_time: time, duration_minutes: durationMinutes, notes,
  }).eq('id', exerciseId)
